...
/* 按键引脚定义
	按键                        引脚名                  引脚号               
	K1                      PINNAME_TXD_AUX             29
*/
/* 定义按键引脚 */
#define    K1_PINNAME           PINNAME_TXD_AUX
...
*********************************************************************************************************
*/
static u8 IsKeyDown1(void) 
{
    /* 低电平表示按键按下 */
    if (Ql_GPIO_GetLevel(K1_PINNAME) == 0) 
		return 1; 
    else 
		return 0;       
}
...
{
    /* 初始化K1引脚 ( 输入,带上拉 ) */
    Ql_GPIO_Init(K1_PINNAME, PINDIRECTION_IN, PINLEVEL_HIGH, PINPULLSEL_PULLDOWN);
    APP_DEBUG("<-- Initialize K1 pin (PINNAME_TXD_AUX): input, HIGH level, PULL up -->\r\n");
}
